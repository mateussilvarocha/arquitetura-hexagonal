...
class Tela:
    def __init__(self, largura=1300, altura=600):
        ...
        self.altura = altura
        self.tela = pygame.display.set_mode((self.largura, self.altura), pygame.RESIZABLE)  # Tela redimensionável
    ...
    def redimensionar(self, nova_largura, nova_altura):
        """Atualiza as dimensões da tela e ajusta elementos se necessário."""
...
class Elementos:
    ...
    def criar_buttons(self):
        dicionario_buttons = {
            'empregar': {
                'x': int(self.ui.tela.largura * 0.1),  # Proporcional à largura da tela
                'y': int(self.ui.tela.altura * 0.3),   # Proporcional à altura da tela
                'largura': int(self.ui.tela.largura * 0.15),
                'altura': int(self.ui.tela.altura * 0.1),
                'texto': 'Empregar',
                'cor': (241, 196, 15),
            ...
            'trabalhar': {
                'x': int(self.ui.tela.largura * 0.3),
                'y': int(self.ui.tela.altura * 0.3),
                'largura': int(self.ui.tela.largura * 0.15),
                'altura': int(self.ui.tela.altura * 0.1),
                'texto': 'Trabalhar',
                'cor': (241, 196, 15),
            ...
            'exit': {
                'x': int(self.ui.tela.largura * 0.1),
                'y': int(self.ui.tela.altura * 0.5),
                'largura': int(self.ui.tela.largura * 0.15),
                'altura': int(self.ui.tela.altura * 0.1),
                'texto': 'Sair',
                'cor': (255, 0, 0),
    ...
    def criar_caixas(self):
        opcoes = {
            "dormir": {
                ...
                'altura': 30,
                'opcoes': [
                    {"nome": "Chão", "acao": lambda: self.controlador.dormir('chao')},
                    {"nome": "Cama pobre", "acao": lambda: self.controlador.dormir('cama pobre')},
                    {"nome": "Cama_rica", "acao": lambda: self.controlador.dormir('cama rica')}
                ]
            },
            "comer": {
                'x': 220,
                ...
                'altura': 30,
                'opcoes': [
                    {"nome": "Comida_estragada", "acao": lambda: self.controlador.comer('comida_estragada')},
                    {"nome": "Comida_pobre", "acao": lambda: self.controlador.comer('comida_pobre')},
                    {"nome": "Comida_rica", "acao": lambda: self.controlador.comer('comida_rica')}
                ]
            }
        }
...
class UIPygame:
    ...
    def checar_eventos(self, evento):
        ...
        if evento.type == pygame.VIDEORESIZE:
            nova_largura, nova_altura = evento.size
            self.tela.redimensionar(nova_largura, nova_altura)
            # Aqui você pode recalcular a posição/tamanho dos elementos se necessário
            self.elementos = Elementos(self.tela.fonte, self.controlador, self)
