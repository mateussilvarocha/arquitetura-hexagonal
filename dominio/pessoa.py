...
class Pessoa(Subject):
    ...
    def empregar(self, cargo):
        self.criar_cargo = CargoFactory.criar_cargo(cargo, self.gerenciador_eventos)
    ...
    def comer(self, comida):
        if self.itens.comida is None:
            self.itens.comida= ComidaFactory.criar_comida(comida, self.gerenciador_eventos)
        
        self.itens.comida = ComidaFactory.criar_comida(comida, self.gerenciador_eventos)
        if self.itens.cama is not None:
            self.eventos.notificar('comeu', {'alimento': comida})

        else:
            # Notifica um erro se a cama for None
            self.eventos.notificar('erro', {'mensagem': "Erro: comida não foi criada corretamente."})
    
    def dormir(self, cama):
        if self.itens.casa is None:
            self.itens.casa = CasaFactory.criar_casa(cama, self.gerenciador_eventos)
        
        # Verifica se a cama foi corretamente inicializada
        self.itens.cama = CamaFactory.criar_cama(cama, self.gerenciador_eventos)
        if self.itens.cama is not None:
            self.eventos.notificar('dormiu', {'cama': cama})
        else:
            # Notifica um erro se a cama for None
            self.eventos.notificar('erro', {'mensagem': "Erro: Cama não foi criada corretamente."})
